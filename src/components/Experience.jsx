...
We have a wide range of beverages!`,
  ...
  useLayoutEffect(() => {
    ...
    planeOutTl.current.to(airplane.current.position, {
      duration: 1,
      onComplete:function(){audioplay.repeat()},
      z: -1000,
    });
  ...
  useFrame(({ camera, size }) => {
    if (muteButtonRef.current) {
      // Position the mesh in the top-right corner of the screen
      const x = size.width / 1 - 70; // 70px from the right edge
      const y = size.height / 5.5 - 70; // 70px from the top edge
      ...
      muteButtonRef.current.position.copy(vector);
    }
  });
  ...
  return useMemo(
    () => (
      <>
        ...
        <mesh
          ...
          onClick={toggleAudio}
        >
          <circleGeometry args={[0.01, 32]} /> {/* Smaller circle (radius: 0.1) */}
          <meshStandardMaterial
            transparent={true} // Make the material transparent
            opacity={0} // Set the opacity (0 = fully transparent, 1 = fully opaque)
            color="white" // Keep the existing color (or match your design)
          />
        </mesh>
    ...
    [isMuted, toggleAudio] // Add isMuted and toggleAudio to the dependency array]
